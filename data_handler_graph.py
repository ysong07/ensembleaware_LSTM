...
class BouncingMNISTDataHandler(object):
  ...
  def __init__(self, data_pb):
    ...
    try:
      ...
      f = h5py.File('/work/ys1297/ecog/source/unsupervised_LSTM/unsupervised-videos/datasets/mnist.h5')
    except:
...
class VideoPatchDataHandler(object):
  def __init__(self, data_pb):
    ...
    self.batch_size_ = data_pb.batch_size
    self.image_size_ = data_pb.image_size
    self.data_file_ = h5py.File(data_pb.data_file)[data_pb.dataset_name]

    # data_stats normalize the data
    self.stats = pkl.load(open(data_pb.mean_file,'r'))
    try:
      self.labels_ = h5py.File(data_pb.data_file)[data_pb.dataset_name+'_label']
      #self.label_ = h5py.File(data_pb.data_file)["train_label"]
      #self.label_ = h5py.File(data_pb.data_file)["valid_label"]
    except:
      print "error"  
    ...
    if self.is_color_:
      ...
      self.frame_size_ = self.image_size_

    self.data_ = np.float32(self.data_file_[:])

    self.dataset_size_ = self.data_.shape[0]
    self.id_= 0
    self.down_sample_rate_ = 10
    self.indices_ = np.arange((self.dataset_size_-self.seq_length_)/self.down_sample_rate_)
    print self.indices_.max()
    
    np.random.shuffle(self.indices_)
    try:
      kit = scipy_io.loadmat('/work/ys1297/ecog/source/unsupervised_LSTM/unsupervised-videos/datasets/reconstruct_kit.mat')
      self.bad_channel_id = kit['bad_channel_id']-1
      self.good_channel_id = kit['good_channel_id']-1
      self.eig_Vec = kit['eig_Vec']
      self.M_matrix = kit['M']
    except:
      print "error" 
  def GetBatchSize(self):
  ...
  def GetBatch(self, verbose=False):
    minibatch = np.zeros((self.batch_size_, self.seq_length_, self.image_size_), dtype=np.float32)
    minibatch_label = np.zeros((self.batch_size_,1),dtype=np.float32)
   
    for j in range(self.batch_size_):
      minibatch[j,:,:] = self.data_[self.indices_[self.id_]*self.down_sample_rate_:self.indices_[self.id_]*self.down_sample_rate_+self.seq_length_,:]
      minibatch_label[j] = self.labels_[self.indices_[self.id_]*self.down_sample_rate_+self.seq_length_]
      self.id_ += 1
    ...
    return minibatch.reshape(self.batch_size_, -1), minibatch_label

 # """ thresholded version"""
 # def GetBatch(self, verbose=False,threshold=0.45):
 #   minibatch = np.zeros((self.batch_size_, self.seq_length_, self.image_size_), dtype=np.float32)
 #   minibatch_label = np.zeros((self.batch_size_,1),dtype=np.float32)
 #   count = 0 
 #   while True:
 #     temp_data = self.data_[self.indices_[self.id_]*self.down_sample_rate_:self.indices_[self.id_]*self.down_sample_rate_+self.seq_length_,:]
 #     if temp_data[0:temp_data.shape[0],:].max()>threshold:
 #       minibatch[count,:,:] = self.data_[self.indices_[self.id_]*self.down_sample_rate_:self.indices_[self.id_]*self.down_sample_rate_+self.seq_length_,:]
 #       minibatch_label[count] = self.labels_[self.indices_[self.id_]*self.down_sample_rate_+self.seq_length_]
 #       count = count+1
 #       if count >= self.batch_size_-1:
 #         self.id_+=1
 #         break
 #     self.id_ += 1
 #     if self.id_ >= (self.dataset_size_-self.seq_length_-1)/self.down_sample_rate_:
...
##	print "restarting"
 ...
 #   return minibatch.reshape(self.batch_size_, -1), minibatch_label  
  ...
  def Get_ordered_Batch(self,threshold= 0.65):
    minibatch = np.zeros((self.batch_size_, self.seq_length_, self.image_size_), dtype=np.float32)
    minibatch_label = np.zeros(self.batch_size_)
    minibatch_thresh_mask = np.zeros(self.batch_size_)
    for j in range(self.batch_size_):
      minibatch[j,:,:] = self.data_[self.id_*self.down_sample_rate_:self.id_*self.down_sample_rate_+self.seq_length_,:]
      minibatch_label[j] = self.labels_[self.id_*self.down_sample_rate_+self.seq_length_]
      if self.data_[self.id_*self.down_sample_rate_:self.id_*self.down_sample_rate_+self.seq_length_,:].max()>=threshold:
        minibatch_thresh_mask[j] = 1
      else:
	minibatch_thresh_mask[j] = 0
      self.id_ += 1
      if self.id_ >= (self.dataset_size_-self.seq_length_-1)/self.down_sample_rate_:
        self.id_ = 0
        minibatch_label =None 
    return minibatch.reshape(self.batch_size_, -1),minibatch_label,minibatch_thresh_mask
 
  def graph_2_tensor(self,unfiltered,seq_length):
    unfiltered = unfiltered.reshape(seq_length,-1,order='F')
    """ rescale coefficient""" 
    unfiltered = unfiltered*1300-300
    unfiltered_incremental = np.zeros([unfiltered.shape[0],self.good_channel_id.size])
    unfiltered_incremental[:,0:unfiltered.shape[1]] = unfiltered
    data = np.zeros([unfiltered.shape[0],self.good_channel_id.size+self.bad_channel_id.size])
    for i in range(unfiltered.shape[0]):
      data[i,self.good_channel_id] = np.dot(self.eig_Vec,unfiltered_incremental[i,:])
      data[i,self.bad_channel_id] = np.dot(self.M_matrix,data[i,self.good_channel_id].squeeze())
    """rescale data """
    #stats = self.stats
    #data = np.float32(data)
    #data = (data-stats['min'])/(stats['max']-stats['min'])
    #data[data<0]=0
    #data[data>1]=1
    return data
  ...
  def DisplayData_Ecog(self, data, rec=None, fut=None, fig=1, case_id=0, output_file=None):
    output_file1 = None
    output_file2 = None
    scipy_io.savemat('test_graph.mat',{'data':data,'fut':fut,'rec':rec})
    if output_file is not None:
      name, ext = os.path.splitext(output_file)
      output_file1 = '%s_original%s' % (name, ext)
      output_file2 = '%s_recon%s' % (name, ext)
    ...
    if self.is_color_:
      ...
      data_reconstruct = self.graph_2_tensor(data[case_id,:],self.seq_length_)
      data = data_reconstruct.reshape(-1, 18+6, 20+6,order = 'F')
      data = data[:,3:18+4-1,3:20+4-1]
	...
	rec_reconstruct = self.graph_2_tensor(rec[case_id,:],self.seq_length_/2)
        rec = rec_reconstruct.reshape(-1,  18+6,20+6,order ='F')
        rec = rec[:,3:18+4-1,3:20+4-1]
	...
	fut_reconstruct = self.graph_2_tensor(fut[case_id,:],self.seq_length_/2)
        fut = fut_reconstruct.reshape(-1, 18+6, 20+6,order ='F')
        fut = fut[:,3:18+4-1,3:20+4-1]
    ...
    """ new plot method """
    num_rows = 3
    plt.figure(2*fig, figsize=(self.num_frames_, 3))
    plt.clf()
    #plt.figure(1)
    for i in xrange(self.seq_length_):
      plt.subplot(num_rows, self.seq_length_, i+1)
