...
class LSTMCombo(object):
  ...
  def Show_fix_range(self,data,batch_id,output_dir=None):
    ...
    for rand_index in range(v_cpu.shape[0]):
      output_file = './imgs_ensemble1/Ecog_{}.pdf'.format(rand_index)
...
def main_sub(board,rank):
  board = LockGPU(board=board)
  model = ReadModelProto(model_name)
  cm.CUDAMatrix.init_random(200)
  np.random.seed(200)
  lstm_autoencoder1 = LSTMCombo(model)

  if rank<=3:
    data_proto_file = 'datasets_joint4041/ecog_'+str(rank)+'.pbtxt'
  else:
    data_proto_file = 'datasets_joint4041/ecog_'+str(rank-3)+'.pbtxt'
  train_data = ChooseDataHandler(ReadDataProto(data_proto_file))
  valid_data = ChooseDataHandler(ReadDataProto('datasets_joint4041/ecog_valid.pbtxt'))
  lstm_autoencoder1.Train(train_data,valid_data)  
...
if __name__ == '__main__':
 ...
 # model_name = '/work/ys1297/ecog/source/unsupervised_LSTM_compile/unsupervised-videos/models_joint_4041_ensemble/lstm_combo_2layer_ecog_' +str(rank)+'.pbtxt'
  model_name = '/scratch/ys1297/ecog/source/models_33/lstm_combo_2layer_ecog_' +str(rank)+'.pbtxt'  
  print model_name
  if rank <=3:
    #import threading
    #threading.Thread(target = main_sub,args=(0,rank)).start()
    #threading.Thread(target = main_sub,args=(1,rank+4)).start()
   ...
   # pass
    ...
    #data_proto_file = 'datasets_joint4041/ecog_'+str(rank)+'.pbtxt'
    data_proto_file = 'datasets_33/ecog_'+str(rank)+'.pbtxt'
    train_data = ChooseDataHandler(ReadDataProto(data_proto_file))
    valid_data = ChooseDataHandler(ReadDataProto('datasets_33/ecog_valid.pbtxt'))
    lstm_autoencoder1.Train(train_data,valid_data)
    #import threading
    #threading.Thread(target = lstm_autoencoder1.Train,args=(train_data,valid_data)).start()
    #
    #
    #board = LockGPU(board=1)
    #model = ReadModelProto(model_name)
    #cm.CUDAMatrix.init_random(100)
    #np.random.seed(100)
    #lstm_autoencoder2 = LSTMCombo(model)

    #data_proto_file = 'datasets_joint4041/ecog_'+str(rank)+'.pbtxt'
    #train_data = ChooseDataHandler(ReadDataProto(data_proto_file))
    #valid_data = ChooseDataHandler(ReadDataProto('datasets_joint4041/ecog_valid.pbtxt'))
    #threading.Thread(target = lstm_autoencoder2.Train,args=(train_data,valid_data)).start() 
