...
class LSTMClassifier(object):
  ...
  def Validate_Ecog(self, data):
    ...
    for ii in xrange(num_batches):
      ...
      preds[start:end, :] = self.GetPrediction().asarray()[:end-start,:]
      correct+= (preds[start:end, :].argmax(axis=1) == t_cpu).sum()
      start = end
    return correct/float(dataset_size), correct/float(dataset_size)
  ...
  def Train(self, train_data, valid_data=None):
    ...
    for ii in xrange(1, self.model_.max_iters + 1):
      ...
      if validate and ii % validate_after == 0:
        valid_loss, valid_loss_pooled = self.Validate_Ecog(valid_data)
