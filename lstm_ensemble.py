...
class LSTMCombo(object):
  ...
  def ensemble_forward(self,ii):
    ...
    # Compute Performance.
    this_loss_dec, this_loss_fut = self.GetLoss()
    sys.stdout.write(' Dec %.5f Fut %.5f \n' % (this_loss_dec, this_loss_fut))
...
if __name__ == '__main__':
  ...
  model = ReadModelProto('models_joint_4041_ensemble/lstm_combo_2layer_ecog_0_deploy.pbtxt')
  cm.CUDAMatrix.init_random(200)
  np.random.seed(200)
  lstm_autoencoder_0 = LSTMCombo(model)
  train_data_0 = ChooseDataHandler(ReadDataProto('datasets_joint4041/ecog.pbtxt'))

  board = LockGPU(board=board_id)   
  model = ReadModelProto('models_joint_4041_ensemble/lstm_combo_2layer_ecog_1_deploy.pbtxt')
  cm.CUDAMatrix.init_random(200)
  np.random.seed(200)
  lstm_autoencoder_1 = LSTMCombo(model)
  train_data_1 = ChooseDataHandler(ReadDataProto('datasets_joint4041/ecog.pbtxt'))  

  #board = LockGPU(board=board_id)   
  #model = ReadModelProto('models_joint4041_ensemble2/lstm_combo_2layer_ecog_deploy.pbtxt')
  #cm.CUDAMatrix.init_random(300)
  #np.random.seed(300)
  #lstm_autoencoder_2 = LSTMCombo(model)
  #train_data_2 = ChooseDataHandler(ReadDataProto('datasets_joint4041/ecog.pbtxt'))  

 # board = LockGPU(board=board_id)   
 # model = ReadModelProto('models_joint4041_ensemble3/lstm_combo_2layer_ecog_deploy.pbtxt')
 # cm.CUDAMatrix.init_random(300)
 # np.random.seed(300)
 # lstm_autoencoder_3 = LSTMCombo(model)
 # train_data_3 = ChooseDataHandler(ReadDataProto('datasets_joint4041/ecog.pbtxt'))
  ...
  for ii in xrange(1, 10000000): 
    ls_dec_0, ls_fut_0 = lstm_autoencoder_0.ensemble_forward(ii)  
    ls_dec_1, ls_fut_1 = lstm_autoencoder_1.ensemble_forward(ii)
    if ls_dec_0+ls_fut_0 < ls_dec_1 + ls_fut_1:
      sys.stdout.write(' update 0')
      lstm_autoencoder_0.BpropAndOutp()
      lstm_autoencoder_0.Update()
    else: 
      sys.stdout.write(' update 1')
      lstm_autoencoder_1.BpropAndOutp()
      lstm_autoencoder_1.Update()
    if ii % 20000 ==1 :
      lstm_autoencoder_0.Save('models_joint_4041_ensemble/train_0/ensemble_MPI_%s' % ii)
      lstm_autoencoder_1.Save('models_joint_4041_ensemble/train_1/ensemble_MPI_%s' % ii)     
    sys.stdout.write('\n')
